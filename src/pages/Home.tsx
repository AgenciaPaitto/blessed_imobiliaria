  ...
  useEffect(() => {
    fetch('/api/properties')
      ...
      .then(data => {
        ...
        setSaleProperties(data.filter((p: Property) => p.status === 'venda').slice(0, 3));
        setRentProperties(data.filter((p: Property) => p.status === 'locacao').slice(0, 3));
      });
  }, []);
  ...
  return (
    <div>
      ...
      <section className="py-20 bg-white border-b border-gray-100">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Imóveis à <span className="text-gold-gradient">Venda</span></h2>
            <p className="text-gray-600">Confira as últimas novidades para compra na região de Tangará da Serra.</p>
          </div>
          ...
          <div className="mt-12 text-center">
            <Link 
              to="/imoveis?status=venda" 
              className="inline-flex bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-lg shadow-md hover:shadow-lg transition active:translate-y-[1px]"
            >
              Ver todos à venda
            </Link>
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Imóveis para <span className="text-gold-gradient">Locação</span></h2>
            <p className="text-gray-600">As melhores casas e apartamentos para alugar com condições facilitadas.</p>
          </div>
          ...
          <div className="mt-12 text-center">
            <Link 
              to="/imoveis?status=locacao" 
              className="inline-flex bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-lg shadow-md hover:shadow-lg transition active:translate-y-[1px]"
            >
              Ver todos para locação
            </Link>
          </div>
        </div>
      </section>
